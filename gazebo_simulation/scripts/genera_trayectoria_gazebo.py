...
vel_msg = Twist()
error_msg = Pose2D()
t = 0.0; t0 = 0.0; V_max = 1; W_max = 0.5; #Timer, initial time, maximum velocities [m/s, rad/s], respectively, for a burger type
T = 100.0; k = 1; #Trajectory period, controller gains kx = ky = k
...
def velocity_controller(): #Function to generate the desired trajectory and to compute the signals control
	global ex, ey, V, W, Xd,Yd, tray #Indicate that some variables are global to be used in the main_function

	if(tray == '2'):
		#######################
		#Coordenadas lemniscata
		#######################
		a = 0.05; b = 0.025; X0 = 0; Y0 = 0; w = 2*pi/T
...
def main_function():
	...
	rate = rospy.Rate(25) #Node frequency (Hz)
	counter = 0
