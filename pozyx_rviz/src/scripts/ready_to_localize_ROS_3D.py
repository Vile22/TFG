...
from pypozyx import (POZYX_POS_ALG_UWB_ONLY, POZYX_2_5D,POZYX_2D, POZYX_3D, Coordinates, POZYX_SUCCESS, PozyxConstants, version,
...
if __name__ == "__main__":
    ...
    anchors = [DeviceCoordinates(0x686f, 1, Coordinates(0, 0, 270)),
            DeviceCoordinates(0x6866, 1, Coordinates(5410, 4250, 2150)),
            DeviceCoordinates(0x6831, 1, Coordinates(-140, 4350, 2160)),
            DeviceCoordinates(0x6854, 1, Coordinates(6000, 0, 840))]
    ...
    try:
        ...
        while not rospy.is_shutdown():
            coords = Coordinates()
            status = pozyx.doPositioning(coords, dimension, height, algorithm, remote_id)
            if status == POZYX_SUCCESS:
                rospy.loginfo("X: %f Y: %f Z: %f" % (float(coords.x), float(coords.y), float(coords.z)))
                pub.publish(Point32(coords.x, coords.y, coords.z))
                #bag.write('pozyx_coords', Point32())
            rate.sleep()
